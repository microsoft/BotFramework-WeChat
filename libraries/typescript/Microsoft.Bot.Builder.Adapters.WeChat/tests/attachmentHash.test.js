...
describe('My hash library', () => {
    it('should be able to computer hash correctly', () => {
        assert.equal(attachmentHash.computeStringHash(testString), 'e16b52d76ad74bb8d4b507515cd9adb8');
    });
    it('shoule be same result', () => {
        assert.equal(attachmentHash.computeBytesHash(bytes), attachmentHash.computeStringHash(testString));
    });
});